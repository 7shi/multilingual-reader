...
- **「傾向の分析」の追記**: 評価ログから得られた固有の傾向（例：「話者タグの脱落が頻発」「英単語や漢字が混入」など）を、各言語の「傾向の分析」に記入します。
...
### 2.1 比較表の更新
- 「翻訳モデル間の比較」セクションにある表（`| 言語 | ... | 傾向の分析 |`）に対象言語の行を反映します。
- 各モデルの代表スコア（基本の `SCORES.txt` に記録された数値）を記入し、その行の **最大スコアのセル** を太字（`**`）で強調します。最大値が同点の場合は、同点のセルをすべて太字にします。
- `傾向の分析` カラムの書き方は [2.2](#22-傾向の分析の記入方針) を参照します。
- 比較表のスコア行は `uv run examples/tr/generate_compare_rows.py` で生成します。
- このスクリプトは `generate_compare_rows.py` の `ONDE_MODELS` に列挙された各モデルの `SCORES.txt` を読み込み、各言語のスコアを **行内で降順にソートしたタプル** を内部的に作り、そのタプルを **降順比較** して行順を決めます。同じタプルになった場合は言語コード昇順で安定化します。
- スクリプトの出力はモデル列のスコア行のみです。言語名は日本語表記（`LANGUAGES` の `ja`）で出力されるため、そのまま貼り付けられます。`傾向の分析` は出力されないため、その列だけ LLM または手作業で補います。

### 2.2 「傾向の分析」の記入方針

比較表の `傾向の分析` カラムには、**スコアを見れば分かることは書きません**。順位や品質の高低（「gemma4 と qwen3.6 が高品質」「3モデルが拮抗」「gpt-oss だけが可読域」など）は数値の言い換えでしかないため、その分の文字数を具体的な症状に充てます。

- **書くこと**: スコアの数値からは読み取れない、訳文で実際に起きている現象。話者タグの脱落、他言語・漢字の混入、造語や非存在語、正書法の崩れ、生成ループ、出力の途中脱落、特定の用語の誤訳など、どのモデルで何が起きたかを具体的に記します。
- **書かないこと**: 「高品質」「拮抗」「相対的に良い」「崩壊」といった、スコアの大小をなぞるだけの評価語。
- **書くことが無ければそのままで良い**: 特異な症状が報告されていない言語（全モデルが安定している言語など）は、無理に埋めず既存の文面を維持します。
