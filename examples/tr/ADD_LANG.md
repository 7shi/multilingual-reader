...
`onde/gemma4/`、`onde/gpt-oss/`、`onde/qwen3.6/` の各ディレクトリについて、以下の作業を行います。
...
- **対象言語の反映**: ファイル上部の対象言語リストに対象言語（例: `朝鮮語 (ko)`）を反映します。
- **品質カテゴリの分類**: 評価スコア（基本となるローカルOllama版や自身の評価スコア）を元に、対象言語を「高品質」「実用レベル」「中〜低品質」「致命的な欠陥」のいずれかのカテゴリに追加します。
- **特記事項（ハルシネーション等）の追記**: 評価ログから得られた固有の傾向（例：「話者タグの脱落が頻発」「英単語や漢字が混入」など）を、既存のハルシネーションの箇条書きや各言語の品質傾向の記述の中に追記します。
- **新旧の区別をしない**: 文中で「新規追加分」「追加した言語」などの表現は使わず、既存言語と同列の対象として記述します。
...
### 2.1 比較表の更新
- 「翻訳モデル間の比較」セクションにある表（`| 言語 | gemma4 | gpt-oss | qwen3.6 | 傾向の分析 |`）に対象言語の行を反映します。
...
- このスクリプトは `onde/{gemma4,gpt-oss,qwen3.6}/SCORES.txt` を読み込み、各言語の3スコアを **行内で降順にソートした3値タプル** を内部的に作り、そのタプルを **降順比較** して行順を決めます。同じタプルになった場合は言語コード昇順で安定化します。
- スクリプトの出力は `言語 | gemma4 | gpt-oss | qwen3.6` のスコア行のみです。`傾向の分析` は出力されないため、生成された行をもとに LLM または手作業で比較表へ書き写します。
