...
`examples/finetuning-es.txt`（Gemini 3.0 Pro による参照訳）を同じパイプライン（3回評価・中央値）で採点した。
...
3回とも97点で評価ブレがゼロ。Phase B 上位3モデル（qwen3.6-27b, gemma4-31b, gemma4-26b）と同点。

readability と fluency で一律 -1 が入るパターンは上位モデル全般に共通しており、この評価者では **97点が実質的な上限（天井）** と考えられる。97点モデルはほぼ満点と見なしてよい。
...
**注記:** `Tech Relámpago` はポッドキャストをスペイン語原文と仮定して翻訳したものであり、今回の実験ではその前提を評価者に伝えていない。評価者はフランス語からの翻訳として採点しているため、この指摘は前提条件の違いによるものである。

**まとめ:** 参照訳は上位 LLM モデルと同水準。`antisèche` を `guía rápida`（簡単なガイド）と訳しているが、Phase B 上位モデルが採用した `chuleta`（カンニングペーパー）の方が原義に近いとの指摘は今回の評価では出なかった。
