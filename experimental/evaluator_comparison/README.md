...
- 「highly readable」「flows naturally」など詳細な評価コメントを生成（幻覚）
- gpt-oss系は正しく0点評価
...
- 使い物にならない翻訳（英語混入、JSON混入、メタコメンタリー）に67-96点を付与
- 致命的な欠陥を軽視、部分評価（良好な部分や最後の部分のみ評価）

**最も重大な例（mixtral-8x7b-tr5-10）**:
- 実態: 43行中10-15行のみ正常、残りは英語メタコメンタリーで汚染、使い物にならない
- GPT評価: **96点**「高品質...19.2/20」← 完全なハルシネーション
- Gemini評価: 4点「翻訳ではない」← 妥当
- 問題: 最後の1行（2.3%）だけを見て全体を評価

**評価傾向の比較**:
| 評価者 | 傾向 | ハルシネーション発生条件 |
|--------|------|----------------------|
| **Gemini** | 厳格 | 翻訳が存在しない（空、CoTのみ） |
| **GPT-OSS-120B** | 寛容 | 翻訳が破綻（大量汚染、最後だけ正常） |
...
**Geminiのハルシネーション（9件）**:
- qwen3-30b-nt系列: 翻訳が存在しないファイルに63-100点を付与
- gpt-oss系は正しく0点評価
- 平均誤差: +78.6点

**GPT-OSS-120Bのハルシネーション（4件以上）**:
- mixtral-8x7b-tr5-10など: 使い物にならない翻訳に67-96点を付与
- Geminiは正しく4-25点評価
- 最大誤差: +92点（mixtral事例）

詳細は [REPORT.md](REPORT.md) の「🔴 追加発見: GPT-OSS-120Bのハルシネーション（幻覚）による誤評価」セクションを参照
...
**両評価者の問題点**:
1. **Gemini**: 翻訳が存在しないケースで幻覚（9件、平均+78.6点の過大評価）
2. **GPT-OSS-120B**: 翻訳が破綻しているケースで幻覚（4件以上、最大+92点の過大評価）

**評価傾向**:
- **Gemini**: 厳格、構造的・内容的欠陥を正確に検出、一部の欠陥で全体を低評価
- **GPT-OSS-120B**: 寛容、致命的欠陥を軽視、部分評価（良好な部分や最後の部分のみ）
...
1. **ハルシネーション対策**: 翻訳の存在確認、入力検証、部分評価の防止
2. **評価基準の改善**: 致命的欠陥の定義、構造的問題の重み付け、実用性の考慮
3. **評価者の選択**: 両評価者に問題があるため単純な移行は不可、用途に応じた使い分けまたは併用を検討
