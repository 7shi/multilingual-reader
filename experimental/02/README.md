...
**サマリー方式の動作（threshold=10, keep=5 の場合）:**

- 10行ごとにサマリーを生成し、サマリー生成後 5行で `system + 最新サマリー + 直近5件` に圧縮する（圧縮は 15, 25, 35, ... 行目）
- 10行目にサマリーを生成し、5件の翻訳を挟んで 15行目で圧縮する。サマリー生成後〜圧縮前の 5件は KV キャッシュが有効。圧縮直後のリクエストは prefix が変わるため cold start になり、以降は新しい prefix でキャッシュが再開される（詳細 → [debug2/README.md](debug2/README.md)）
