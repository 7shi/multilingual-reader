...
- **[13/](13/)**: Runs the System One evaluator over the corpus's top two translators (`gpt-5.6-luna`, `union-alpha`) in all 67 languages, one run each, against the old scheme's own scores for the same translations. Experiments 11 and 12 both rest on the same 8 targets, picked for being the ones the old scheme handled worst; this is the same comparison at n=67 per translator, on the slice where separation is hardest — the old scheme ranks the pair 0.22 points apart. Both rubrics are carried here as frozen copies rather than imports, since each was the variable its own experiment tested.
  ...
  - Still untested: the bottom of the scale, and the middle of the corpus. Both are one 134-evaluation run each

- **[14/](14/)**: Twelve runs, 718 phrases, and a wording that is settled. `trtools trend` writes each language's one-line trend phrase by summarizing three generative evaluations' prose, and the System One evaluator returns none, so the column has no source once experiment 13's migration lands. The replacement writes the phrase from the translation itself, and what steers it is the level of the weakest criterion alone — a second axis pointing at whichever criteria sat below the record's own mean changed nothing two repeats of one prompt did not change by more, and was dropped
  - **The genre was wrong for six runs.** Asking for the lines that fall short produced fabricated locations on translations with nothing to find; measuring the 1,072 entries of the column being replaced showed it never pointed at all — median five words, a quotation mark in 2%, a line number in none. From run 7 the task is to characterize the translation as a whole, and from run 9 it is the old prompt's own task, with the writer's reasoning standing in for the evaluator's prose it used to summarize
  - **A commercial writer is a yardstick, not a candidate**: every phrase sends the whole original and translation, so one pass over the corpus is ~4.5M input tokens. The writer is a local model, and how much of the yardstick a local model keeps is what runs 11–12 measure
  - **Thinking buys obedience, not accuracy.** Turning it off leaves the false rate unchanged at 7 phrases in 84, but the ban on quoting stops being applied — 0–5% with it, 19–29% without — and the adopted variant destabilizes. It costs 117 seconds a phrase against 2.1, which is 35 hours against 37 minutes for a full pass
  - Still open: which local writer, since only one has been measured; and whether the column would rather have the cheap, stabler, vaguer phrases the un-thought variant produces
