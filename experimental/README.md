...
- **統計的信頼性**: 3回評価の中央値を使用（`tr-agg`）
- **評価結果**: [SCORES.txt](SCORES.txt)

**使用方法**:
```bash
uv run tr-eval --original 原文.txt --translation 翻訳文.txt \
...
```bash
# 詳細表示
uv run tr-agg evaluation-*-*.json --verbose

# 簡潔表示（中央値のみ）
uv run tr-agg evaluation-*-*.json
```
...
**評価後の集計フロー**:
- `tr-agg`: 最終的なスコアを集約し、`SCORES.txt`に保存
