...
Detailed tables and per-item breakdowns are recorded in [SCORES.md](SCORES.md) (generated by [agg50.py](agg50.py)). All 72 calls in both `evals/` and `evals-nt/` completed without schema failures.
...
Because prompts provide plain text without line numbers, models frequently hallucinated line references in the `evidence` field (e.g. citing lines 15 and 64 when real defects were on lines 15, 20, 72, 78, 90). The question remains whether producing evidence aids the verdict or introduces unnecessary noise.

---

## 5. Future Work & Next Steps

1. **Evaluate `evals-ne/` (Evidence ON vs OFF)**:
   - Run thinking ON with `--no-evidence` to isolate whether `qwen3.6`'s score drop in `evals-nt/` was caused by disabling thinking or removing the evidence field, and assess speedups for other models.
2. **Implement Catastrophic Failure Gating**:
...
3. **Line-Numbered Evaluation Prompts**:
...
5. **Mainline Integration into `trtools`**:
