...
Detailed tables and per-item breakdowns are recorded in [SCORES.md](SCORES.md) (generated by [agg50.py](agg50.py)). All 120 calls in each of `evals/`, `evals-nt/`, and `evals-ne/` completed without schema failures.
...
**Cheapest decisive test.** Run `bonsai2-27b` and `qwen3.8` over all 67 languages under the new scheme with `qwen3.6` as evaluator -- 134 calls, no commercial API, roughly 9 hours at the measured 245s per thinking call. If the new scheme plus a local evaluator cannot reproduce the 26-point separation the old scheme found, "new scheme + local" is unusable even in aggregate, and the question above is settled without paying for a 67-language commercial run.
...
3. **Fix the Local Evaluators' `partial` Band**:
   - The ground-truth check localizes the problem to the unused 1-3 line middle band. Give the rubric explicit counted anchors (e.g. "state the number of offending lines, then map 0 / 1-3 / 4+ to the verdict") and re-run `gemma4:31b` and `qwen3.6` to see whether `partial` usage and `a01` accuracy both move.
...
7. **Run the Quantization Separation Test**:
   - Evaluate `bonsai2-27b` and `qwen3.8` over all 67 languages under the new scheme with `qwen3.6` as evaluator (134 calls, local only). The old scheme separated them by 26 points in 62 of 67 languages; whether the new scheme plus a local evaluator reproduces that decides if "new scheme + local" is usable in aggregate. See "Provisional Position".
