...
gemma4-26b の生成品質は全体的に高く、文意の誤りはほぼなし。主な指摘パターン：

- **スピーカータグ欠落**: momentum-de（22箇所）・transformer-zh（4箇所）・momentum-zh（2箇所）。短い相槌行（"Oh?"・"Okay."・"Hmm."・"Richtig." 等）や、長い発言が話者交代を挟んで続く継続行でタグが落ちる。ドイツ語で特に顕著で、tr-fr で観察された傾向と同様
- **書式の不統一**: onde-ja の全行が「」で括られており他日本語ファイルと不統一。内側の『』を「」に変換して全文書き直し
- **敬語・格変化のミス**: finetuning-de の1箇所（`Dein` → `Ihr`、Sie 敬語の統一）・onde-de の1箇所（`ihr Quadrat der Amplitude` → `das Quadrat ihrer Amplitude`）。いずれも孤立したミスで体系的ではない
- **意味不一致**: onde-zh の1箇所（`怎么了？` → `嗯？`、"Yes?" の応答として不適）
...
EN→JA は旧訳（gemma3n:e4b ベース）が既に高品質のため維持。
