# メモ

[examples/tr/README.md](examples/tr/README.md) にある実験構成や結果概要とは分けて、ここでは判断メモと今後の検討事項だけを残します。

## 知見の要点

### モデルごとの傾向

- `gemma4` (Gemma 4 26B-A4B): 最も安定。コア言語だけでなく、中程度のリソース量がある言語でも文法の骨格と対話の流れを保ちやすい
- `gpt-oss` (GPT-OSS 120B-A5.1B): 意味理解は強いが、翻訳生成では話者タグ脱落や構造崩れが出やすい。読む力は高いが、書く力は安定しない
- `qwen3.6` (Qwen 3.6 35B-A3B): 評価者としては優秀で、崩れた訳文の欠陥もよく見つける。一方で翻訳器としては多言語混入や文字体系の破綻が起きやすい
- `qwen3.8` (Qwen 3.8 27B, dense): 高リソース言語では良好だが、出力が途中で脱落しやすい（本文が消えて話者名だけ並ぶ、文の途中で切れる）。フィンランド語では0点近くまで崩れた
- `muse-glimmer` (27.9B, dense): 中〜低リソースの欧州言語（マケドニア語、ブルガリア語、リトアニア語など）で強いが、文・行の重複が頻発する
- `ox-alpha`（評価用ステルスモデル、期間限定）: 中〜低リソース言語で幅広く安定しており、`gpt-5.6-luna` を除くと単独最多の首位獲得数。一方で中国語・朝鮮語など一部の高リソース言語では話者タグ脱落により伸び悩む
- `gpt-5.6-luna`（クローズドモデル）: 比較対象67言語中54言語で最高スコアを獲得。他モデルで頻発した多言語混入・システムプロンプト漏洩・話者タグ脱落がほぼ観測されず、現時点で最も汎用的かつ安定

### Google製モデル間の傾向差

`gemma4` と `gemini-3.5-flash-lite` はどちらも Google 製ですが、得意・不得意の傾向はむしろ対照的でした。

- 全言語平均はほぼ同水準（gemma4 68.2点、gemini-3.5-flash-lite 64.2点）だが、言語ごとのスコアの相関係数は 0.55 程度と弱く、「同じベンダーだから傾向が似る」とは言えない
- `gemma4` が大きく上回る言語（例: 中国語 96 vs 49、ウクライナ語 93 vs 39、スワヒリ語 82 vs 33）は、`gemma4` が安定して高得点を出す中〜高リソース言語であり、`gemini-3.5-flash-lite` の方は他言語混入や原文残存で崩れやすい
- 逆に `gemini-3.5-flash-lite` が大きく上回る言語（例: エスペラント 83 vs 31、ビルマ語 81 vs 34、マラヤーラム語 82 vs 49、ジョージア語 91 vs 60、バスク語 45 vs 15）は、`gemma4` がコードスイッチングなどで壊滅的に崩れる言語帯にちょうど重なる
- 崩壊パターンも異なる: `gemma4` はハンガリー語で見られたような文中への無関係な単語混入が局所的に起きるのに対し、`gemini-3.5-flash-lite` は特定言語で複数の異なる文字体系がまとまって混入する（例: シンハラ語で他の南アジア言語文字が広範囲に混入）など、崩れ方の質自体が違う
- 同じ Google 系列でも、モデル規模やアーキテクチャ、チューニング方針の違いの方が翻訳傾向を大きく左右すると考えられる

### 挙動の背景についての仮説

- 翻訳品質は言語構造そのものより、学習データ量と標準化されたデジタル資源の充実度に強く依存していそう
- 同じモデルでも特定言語だけ局所的に崩れることがあり、これは言語一般の難しさというより、そのモデル内部の表現空間での混線だと考えるのが自然
- 巨大モデルほど常に有利というわけではなく、低〜中リソース言語では小さめでも安定したモデルの方が実用的なことがある
- 評価能力と生成能力には非対称があり、読む力が高いモデルが書く力でも強いとは限らない

### 言語の分類

調査対象の 67 言語について、モデル選択の目安を示します。1 つの言語が複数のモデルで異なる得点帯に分類されることがありますが、太字はそのモデルが最高得点だったことを示します。

**gemma4**:
- 高品質（90点以上）: **フランス語**、**日本語**、**ベトナム語**、**ロシア語**、**ペルシア語**、**中国語**、**アラビア語**、**朝鮮語**、**フィンランド語**、**ドイツ語**、スペイン語、**ウクライナ語**、**セルビア語**、ポルトガル語、イタリア語、**デンマーク語**、**トルコ語**、**ポーランド語**、**アルメニア語**
- 実用範囲（80〜89点）: **ノルウェー語**、**クロアチア語**、**ヘブライ語**、スウェーデン語、**インドネシア語**、**マレー語**、**アルバニア語**、**アフリカーンス語**、**スワヒリ語**、**ブルガリア語**
- 中品質（60〜79点）: オランダ語、ガリシア語、チェコ語、**アゼルバイジャン語**、**マケドニア語**、**モンゴル語**、**ヒンディー語**、カタルーニャ語、**ウルドゥー語**、**タガログ語**、**ラーオ語**、**マラーティー語**、スロバキア語、ルーマニア語、**テルグ語**、ネパール語、**ジョージア語**

**gpt-oss**:
- 高品質（90点以上）: **スペイン語**、フランス語、**カタルーニャ語**、イタリア語、日本語、ベトナム語、ポルトガル語
- 実用範囲（80〜89点）: **スウェーデン語**、ウクライナ語、オランダ語、アラビア語、ドイツ語、トルコ語、アフリカーンス語、ガリシア語、**ブルガリア語**、**チェコ語**
- 中品質（60〜79点）: ロシア語、中国語、インドネシア語、デンマーク語、ハンガリー語、**ネパール語**、マケドニア語、ペルシア語、**スロベニア語**、**インターリングア**、**スロバキア語**、ウルドゥー語、ノルウェー語、マレー語、**ベラルーシ語**、ポーランド語、タガログ語、**ギリシャ語**、朝鮮語、ヘブライ語、アゼルバイジャン語、フィンランド語、セルビア語、ラトビア語、マラーティー語

**qwen3.6**:
- 高品質（90点以上）: フランス語、スペイン語、**ポルトガル語**、**イタリア語**、**ルーマニア語**、ベトナム語、ロシア語、**オランダ語**
- 実用範囲（80〜89点）: ペルシア語、ドイツ語、**ガリシア語**、中国語、ノルウェー語、セルビア語、クロアチア語、**ハンガリー語**、スウェーデン語
- 中品質（60〜79点）: カタルーニャ語、アフリカーンス語、**リトアニア語**、日本語、デンマーク語、朝鮮語、ウクライナ語、ポーランド語、アルバニア語、ヘブライ語、チェコ語、ベラルーシ語、**ラトビア語**、トルコ語、スロバキア語、ギリシャ語

上記は `gemma4`・`gpt-oss`・`qwen3.6` の3モデルの範囲での分類ですが、その後追加した `ox-alpha` と `gpt-5.6-luna` により、当時「どのモデルでも実用水準に達していない」としていた言語（エスペラント、ベンガル語、マラヤーラム語、クメール語、エストニア語、ウェールズ語、カンナダ語、タイ語、アイスランド語、バスク語、タミル語、ビルマ語、シンハラ語、アイルランド語）はすべて解消されました。全モデルの最高点（overall）で見ると、67言語すべてが中品質（60点）以上に達しており、致命的な欠陥（60点未満）の言語は現時点で存在しません。それでも中品質（60〜79点）にとどまっているのは、アイスランド語、ラーオ語、シンハラ語、タガログ語、ウェールズ語の5言語です。

この分類は以下のコマンドで出力しました（`--overall` で全モデル横断の最高点による分類）。

```bash
uv run examples/tr/generate_compare_rows.py classify
uv run examples/tr/generate_compare_rows.py classify --overall
```

### 推敲（他者評価）アプローチの傾向

全 67 言語を対象に、各言語の最高得点ベースラインを qwen3.6 が行単位で推敲する実験（実験09）を実施しました。改善30言語・悪化32言語・変化なし5言語（平均変化 −1.2点）という結果で、推敲の効果は言語・ベーススコアの状態に強く依存します。

**推敲が有効だった言語**（差分 +6 以上かつ推敲後 80点以上）：Bulgarian (97:+17), Hungarian (96:+13), Slovene (95:+22), Azerbaijani (91:+13), Czech (89:+9), Basque (87:+42), Estonian (82:+29), Latvian (82:+17), Macedonian (82:+6), Belarusian (81:+12)

**推敲が効く条件と但し書き**（実験09・10 から）：

- 推敲は「表現の洗練」として機能するため、**意味が通じるが表現が粗い翻訳**に対して有効
- 翻訳が**構造的に破綻している**場合は改善できず悪化する傾向がある
- ベースラインの**完成度が高い**場合は余計な変更で逆効果になるケースもある（bg: 88点 → −1点）
- **ベーススコアが低くても**、意味の骨格が残っていれば推敲が大幅改善をもたらすことがある（実験10: eu 17→59、hu 26→89、sl 56→83、et 30→51）

この非対称な効果のため、推敲の適用可否はスコアの高低だけでは判断できません。

### 運用上の判断

- 品質だけを見れば `gpt-5.6-luna`（クローズド）が最も汎用的だが、オープンウェイトで運用したい場合のベースラインは引き続き `gemma4` が無難。`ox-alpha` は期間限定のテスト用ステルスモデルで、正式版は500B以上と予想され自前運用が困難なため、クラウド利用なら効率の良い `gpt-5.6-luna` を選ぶ方が妥当
- 評価の定規は `qwen3.6` に統一したままの方が比較しやすい
- 低リソース言語では「意味が通じるか」だけでなく、話者タグ保持や他言語混入の有無を重く見るべき
- 推敲（`trtools review`）は中リソース言語で特に有効だが、ベースラインが構造的に破綻している場合や高品質すぎる場合は逆効果になる。スコアだけで判断せず、翻訳の構造的健全性を確認してから適用すること
- プロバイダー差よりモデル差の方が大きいが、JSON の壊れ方や停止の仕方には実行環境差があるので、自動化ではそこだけ別途ケアが必要

## ROCm/Vulkanバックエンドの傾向

評価者(`qwen3.6`)がROCmバックエンドで動作不良を起こす問題を受けて、翻訳・評価それぞれのバックエンドを切り替えた比較を行いました（`examples/tr/onde/qwen3.8/`, `examples/tr/onde/muse-glimmer/`）。

- **評価はROCmで使用不可**: `qwen3.6` を評価者としてROCmで動かすと、存在しない言語混入を報告するハルシネーションや、評価対象の原文・訳文がプロンプトに渡っていないと訴える出力（コンテキスト自体の破損）が発生し、スコアが崩壊する。評価をVulkanに戻すとこれらの異常は解消され、翻訳内容に即した妥当な評価に戻る。評価バックエンドは常にVulkan固定とする。
- **翻訳側のROCm影響はモデル依存**: 評価をVulkanに揃えた上で翻訳バックエンド（ROCm/Vulkan）のみを比較すると、モデルによって挙動が異なる。
  - `qwen3.8`: ROCm翻訳で一部言語（スペイン語・ガリシア語・ルーマニア語）が生成途中で停止し0点化。途切れる直前までは文法・語彙とも自然で、能力不足ではなく生成が完走できない問題と見られる。
  - `muse-glimmer`: ROCm翻訳でも0点言語は発生せず、スコア差は通常のばらつきの範囲内（マレー語など一部言語で±20〜40点の変動はあるが、上昇・低下双方に分布し偏りはない）。
- 運用上は、評価は必ずVulkanで固定し、翻訳をROCmで行う場合はモデルごとに0点言語（生成途中停止）の有無を個別に確認すること。

**推測される原因**: Gemma 4・GPT-OSS 120B・Qwen 3.6 による初期評価（2026年5月）の時点ではROCmでもこの問題は起きていなかった。その後Ollamaに入った大きな変化は、`v0.30.0`（2026年6月）での自前推論エンジン全廃とupstream `llama-server`（llama.cpp）への一本化、およびそれに伴う互換レイヤ（`llama/compat/`）の新設くらいしかなく、この変更がROCmバックエンドでの推論破壊・コンテキスト破損の原因と推測される。

互換レイヤは、旧来のOllama形式GGUFのメタデータ・テンソル名をupstream `llama-server` が期待する形式へ動的変換する仕組みである。この一本化以降にリリースされたモデル（Qwen 3.8・Muse Glimmer）はネイティブ対応済みでこの変換が不要だが、一本化前からある旧モデル（Qwen 3.6・Gemma 4）は変換に依存する。この違いは、実際に前者だけがROCmで正常動作し後者だけが壊れるという観測結果と符合する。

## GPT-OSS 120B の評価外活用

`gpt-oss` は評価タスクでは天井効果があり、主要な評価者には採用していません。一方で、高速推論を活かした補助用途には向いていると思われます。

- 用語確認: 訳語候補の列挙、既存訳語の妥当性チェック
- 背景知識補完: 固有名詞や文化的文脈の説明
- 事前調査: 翻訳前の論点洗い出し

## 将来の検討事項

現在の実験対象は 100 行未満のポッドキャスト台本ですが、最終的には小説 1 冊のような長文翻訳も視野に入れています。その規模になると、新たな機構が必要になるでしょう。

- 階層的コンテキスト管理: 直近区間は詳細要約、遠い区間は粗筋というように粒度を分けて保持したい
- 長距離参照の補助: 単一サマリーでは伏線や人物関係を落としやすいため、関連チャンクを動的に引く仕組みが必要になりそう
- 用語と文体の一貫性: 章をまたぐ訳語統一、人物ごとの話し方、叙述トーンの維持をどう管理するか
- 評価設計の再検討: 長文では行単位評価だけでなく、章単位の整合性や物語理解も見たい

今の実験で固めている用語抽出、要約圧縮などの運用判断は、その先の基盤になることが予想されます。
