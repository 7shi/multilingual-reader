...
### examples/tr-en/extra/: 追加言語 en→(eo, hi, te, kn, tr, et, sr) 翻訳・評価

リソースが少ないマイナー言語への翻訳能力と、LLM特有の挙動（自己評価バイアス、ハルシネーション、プロバイダー間の差異）を深く掘り下げるための検証セット。詳細は `examples/tr-en/README.md` を参照。

#### ディレクトリ構成と検証テーマ
- `extra1/`: ベースライン（翻訳: gemma4:26b, 評価: qwen3.6）。マイナー言語でも文法の骨格を保ち、他の巨大モデルを圧倒する安定性を見せた。
- `extra2/`: 巨大モデルの性能とプロバイダー比較（翻訳: gpt-oss:120b, 評価: qwen3.6）。ローカル（Ollama）とクラウド（OpenRouter）の2系統で実行。
- `extra3/`: 自己評価と他者評価の比較検証（翻訳: qwen3.6, 評価: qwen3.6 / gpt-oss:120b）。

#### 主要な検証結果と知見

1. **翻訳モデルの適性（読み書き能力の非対称性）**:
   - `gemma4:26b` はパラメータ規模が小さいながらも、マイナー言語において文法の骨格を保ち安定した出力をする能力に最も長けていた。
   - `gpt-oss:120b` や `qwen3.6` などの巨大モデルは、マイナー言語の翻訳（書く力）において極めて脆く、「他言語の混入（韓国語やアラビア語など）」「システムプロンプトの漏洩」「漢字の混入」といった致命的な幻覚（ハルシネーション）を頻発させた。
   - 一方で、`qwen3.6` は自らが生成した破綻したテキストを評価（読む力）させた際には、客観的で容赦ない減点を行い、LLMにおける「読み書き能力の著しい非対称性」が実証された。

2. **自己評価バイアスの不在と採点基準の重要性**:
   - LLMに自己評価をさせても必ずしも甘くなる（バイアスがかかる）わけではないことが判明した。
   - 逆に、評価モデルを変更すると「エラーに対する寛容さ（採点基準）」が根本的に変わってしまう（例：gpt-oss は意味が通じれば甘く、qwen は文法破綻に厳しい）ため、品質の相対比較においては「同一の評価モデル（定規）で採点基準を統一すること」が最も重要である。

3. **実行環境（プロバイダー）による挙動の差異**:
   - 同一モデルをローカルとクラウドで比較した結果、エラーの「検知・指摘能力」は環境に依存せず完全に一致した（スコアのブレはサンプリングの揺らぎに過ぎない）。
   - 翻訳時のハルシネーションも環境を問わず発生し、モデル固有の限界であることが確認された。
   - ただし、構造化出力（JSON）には環境依存の差があり、クラウドでは「無意味な値の挿入による出力破損」、ローカルでは「無限ループ（ツール側でリトライ可能）」が発生しやすい傾向が見られた。
