...
[../experimental2/](../experimental2/) のサマリー圧縮方式を発展させ、CoT 対応モデル向けの **ハイブリッドモード** を実装した実験ディレクトリです。

## 背景と動機

experimental2 で `--no-think` を採用した理由は、CoT による履歴肥大とキャッシュミスの回避でした。一方で、要約生成は「複数情報の統合判断」が必要なため CoT が活きる場面です。本実験では：
...
experimental2 の `--summary` / `--schema` / `--no-summary-history` は廃止し固定動作としました。glossary・要約履歴削除は常に有効です。
...
[MODELS.txt](MODELS.txt) の3モデル（qwen3.6-27b、gemma4-26b、gemma4-e4b）について、**翻訳3回 × 評価3回** を実行します。experimental2 では翻訳1回 × 評価3回で「評価ブレ」のみを測っていましたが、本実験ではモデル絞り込みで時間に余裕ができるため、翻訳のブレも測定します。
...
experimental2 の Phase B 結果から、上位3モデルに絞り込みました。いずれも CoT 対応かつ実用的なリソース範囲に収まります。

| モデル | experimental2 スコア | 特徴 |
|---|:---:|---|
| qwen3.6-27b | 97 | think モードでの精度が高い |
| gemma4-26b | 97 | スコア安定性が最高（範囲1）、構造的欠陥ゼロ |
| gemma4-e4b | 95 | 軽量モデルながら情報完全性3回満点 |

experimental2 スコアは Phase B（`--summary glossary`・`--no-think`・翻訳1回・評価3回の中央値）の値。詳細は [../experimental2/README.md](../experimental2/README.md) を参照。
...
[../experimental/](../experimental/) の評価パイプラインをそのまま使用します。詳細は [../experimental2/README.md](../experimental2/README.md) を参照。
...
| [exp2/](exp2/) | experimental2/translate.py | 10 | なし | experimental2 設定での比較用試行。qwen3.6・e4b で急落 run あり |
...
急落はどの設定でも発生しており、特定の設定に依存しない。exp2/（experimental2 設定）でも qwen3.6-27b と gemma4-e4b で急落が起きており、glossary 初期蓄積のブレによるランダムな現象と確認できた。
...
### experimental2 との比較（要約履歴削除の効果）

exp2/ は experimental2 と同じ設定（要約を翻訳履歴に含める）で3回翻訳した結果。10-nt/（要約を履歴から除外）との比較で、ハイブリッドモードの効果を対等に検証できる:
...
各試行のサンプル数（翻訳3回）は統計的な断定には少なく、スコアの差異を設定の効果と言い切るには根拠が薄い。ただし experimental2 の段階では見えていなかった理論的な論点——要約を翻訳履歴に残すことによるスタイル干渉のリスク、小型モデルへの長文脈負荷——が本実験を通して浮かび上がった点に意義がある。
